...
export default function PaginaActas() {
  const { rol: rolUsuario, club: clubUsuario, cargando: authCargando } = useAuth() as any;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
        <div className="lg:col-span-5 space-y-4 md:space-y-6 order-2 lg:order-1">
          <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <select value={partidoSeleccionadoId} onChange={e => { setPartidoSeleccionadoId(e.target.value); setEquipoSeleccionado(null); setJugadorEncontrado(null); setBusqueda(""); }} className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl font-bold outline-none text-sm">
          ...
          {partidoActivo && (
            <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-2 md:gap-3 mb-6">
                <button onClick={() => { setEquipoSeleccionado("local"); setJugadorEncontrado(null); setBusqueda(""); }} className={`p-3 rounded-xl font-bold text-[10px] md:text-sm border transition-all ${equipoSeleccionado === "local" ? 'bg-[#1e3a8a] text-white' : 'bg-slate-50 text-slate-600'}`}>LOCAL</button>
                <button onClick={() => { setEquipoSeleccionado("visita"); setJugadorEncontrado(null); setBusqueda(""); }} className={`p-3 rounded-xl font-bold text-[10px] md:text-sm border transition-all ${equipoSeleccionado === "visita" ? 'bg-[#1e3a8a] text-white' : 'bg-slate-50 text-slate-600'}`}>VISITA</button>
              </div>
                ...
                <div className="space-y-4">
                  <div className="relative">
                    <input 
                      type="text" 
                      value={busqueda} 
                      onChange={e => { setBusqueda(e.target.value); setMostrarResultados(true); setJugadorEncontrado(null); }} 
                      onFocus={() => setMostrarResultados(true)}
                      placeholder="Busca por Nombre o RUT..." 
                      className="w-full p-3 md:p-4 bg-slate-50 border border-slate-300 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-emerald-500 transition-all" 
                    />
