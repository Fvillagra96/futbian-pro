...
export default function ModuloClasificacionLiguilla() {
  ...
  const [configMinimos, setConfigMinimos] = useState<Record<string, number>>({}); // 🚨 NUEVO: Guardamos los mínimos aquí
  ...
  useEffect(() => {
    ...
    // 🚨 NUEVO: Escuchamos la configuración global
    const unsubAsoc = onSnapshot(doc(db, "asociaciones", "san_fabian"), (docSnap) => {
      if (docSnap.exists()) {
        setConfigMinimos(docSnap.data().minPartidosLiguilla || {});
      }
    });
  ...
  // Mostramos al usuario cuál es el mínimo de la serie que está mirando
  const minViendoActual = filtroSerie !== "Todas" ? (configMinimos[filtroSerie] !== undefined ? configMinimos[filtroSerie] : 3) : "Varios";
      ...
      <header className="bg-slate-900 rounded-3xl p-6 md:p-10 shadow-xl text-white relative overflow-hidden border-b-4 border-yellow-500">
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-2 text-xs md:text-sm">Configurado por la Asociación mediante firmas en actas oficiales.</p>
          </div>
          <div className="bg-white/10 px-6 py-3 rounded-xl border border-white/20 text-center">
            <p className="text-[10px] font-bold text-slate-300 uppercase block mb-1">Mínimo Exigido ({filtroSerie})</p>
            <p className="text-2xl font-black text-white leading-none">{minViendoActual} <span className="text-sm font-bold text-slate-400">partidos</span></p>
          </div>
        </div>
          ...
          <table className="w-full min-w-[700px] text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-widest border-b border-slate-200">
                ...
                <th className="p-4 font-black text-center text-[#1e3a8a]">Partidos Jugados</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {jugadoresFiltrados.length === 0 ? <tr><td colSpan={5} className="p-10 text-center font-bold text-slate-400">No se encontraron jugadores.</td></tr> : (
                jugadoresFiltrados.map(j => {
                  ...
                  // 🚨 NUEVO: Evaluamos usando la regla configurada para ESTA serie específica (3 por defecto)
                  const minRequerido = configMinimos[j.serie] !== undefined ? configMinimos[j.serie] : 3;
                  const clasificaPorMerito = jugados >= minRequerido;
                  ...
                  return (
                    <tr key={j.id} className={`hover:bg-slate-50 transition-colors ${estaClasificado ? 'bg-emerald-50/20' : 'bg-red-50/10'}`}>
                      ...
                      <td className="p-4 text-center font-black text-xl text-[#1e3a8a]">{jugados} <span className="text-[10px] font-bold text-slate-400">/ {minRequerido}</span></td>
                      <td className="p-4 text-center">
                        {estaClasificado ? (
                          ...
                          <span className="inline-flex flex-col items-center gap-1 bg-red-100 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-black border border-red-200 uppercase tracking-widest shadow-sm">
                            ❌ Inhabilitado
                            <span className="text-[8px] bg-red-600 text-white px-1.5 rounded-full">Faltan {minRequerido - jugados} partidos</span>
                          </span>
                        )}
                      </td>
                      {rol === 'admin' && (
