...
export default function TribunalDisciplina() {
  ...
  const agregarEventoAuditoria = async (rutJugador: string, nombreJugador: string, equipo: string, tipoEvento: string) => {
    ...
    const nuevoEvento: Evento = {
      id: Date.now().toString(), tipo: tipoEvento, jugador: nombreJugador, rut: rutJugador, equipo: equipo, minuto: "Admin"
    };
    ...
    try {
      ...
      if (tipoEvento === '⚽ Gol') {
        if (equipo === partidoActivo.local) golesL += 1; else golesV += 1;
      } else if (tipoEvento === '⚽❌ Autogol') {
        if (equipo === partidoActivo.local) golesV += 1; else golesL += 1;
      }
      
      // Lógica de sanción automática si el Admin fuerza una tarjeta
  ...
  const aplicarSancion = async (rutJugador: string, nombreJugador: string) => {
    ...
    try {
      ...
      alert(`Sanción actualizada en la ficha del jugador.`);
    } catch (error) { console.error("Error al aplicar la sanción.", error); }
  };

  const handleSancionChange = (rut: string, valor: number) => setSancionesInput(prev => ({...prev, [rut]: valor}));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <select value={partidoSeleccionadoId} onChange={e => setPartidoSeleccionadoId(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl font-bold outline-none text-sm focus:ring-2 focus:ring-red-500">
              <option value="">-- Seleccionar acta --</option>
              {partidosFinalizados.map(p => <option key={p.id} value={p.id}>Fecha {p.fechaNumero} | {p.serie} | {p.local} vs {p.visita}</option>)}
            </select>
          </div>
          {partidoActivo && (
             <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
               ...
               <div className="flex items-center gap-4 text-2xl font-black justify-center my-6 bg-white py-4 rounded-xl shadow-sm">
                 <span className="text-slate-800">{partidoActivo.golesLocal}</span><span className="text-slate-300">-</span><span className="text-slate-800">{partidoActivo.golesVisita}</span>
               </div>
            </div>
          )}
        </div>
           ...
           {partidoActivo ? (
             <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="p-6 border-b border-slate-100">
                   <h3 className="font-black text-slate-800 flex items-center gap-2 mb-4"><span className="text-xl">⏱️</span> Registro de Sucesos (Edición)</h3>
                      ...
                      {partidoActivo.eventos?.length ? [...partidoActivo.eventos].reverse().map((ev, i) => (
                        <div key={ev.id || i} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-xl group">
                          <div className="flex items-center gap-3">
                            ...
                            <div><p className="font-bold text-slate-800 text-xs uppercase">{ev.jugador}</p><p className="text-[10px] font-bold text-blue-600">{ev.equipo} • Minuto: {ev.minuto}</p></div>
                          </div>
                          <button onClick={() => eliminarEvento(ev)} className="px-3 py-1.5 bg-white text-red-500 border border-red-100 hover:bg-red-500 hover:text-white font-bold text-[10px] rounded-lg transition shadow-sm">ELIMINAR ERROR</button>
                        </div>
                             ...
                             return (
                               <div key={i} className="p-3 bg-white border border-slate-200 rounded-xl shadow-sm">
                                  ...
                                  <div className="flex gap-1 mb-3">
                                    ...
                                    <button onClick={() => agregarEventoAuditoria(jugador.rut, jugador.nombre, jugador.equipo, '🟥 Roja Directa')} className="flex-1 bg-slate-100 hover:bg-red-100 border border-slate-200 rounded text-[10px] py-1 font-black transition">🟥 TR</button>
                                  </div>
                                  {suspendido ? (
                                    <div className="bg-red-50 text-red-600 text-[10px] font-black px-2 py-1 rounded text-center border border-red-100">SUSPENDIDO ({datosBD?.partidosSuspendido} FECHAS)</div>
                                  ) : (
                                    <div className="flex gap-2">
                                      <input type="number" min="0" placeholder="Fechas" value={sancionesInput[jugador.rut] || ""} onChange={(e) => handleSancionChange(jugador.rut, parseInt(e.target.value) || 0)} className="w-16 p-1.5 text-xs text-center border border-slate-300 rounded outline-none bg-slate-50" />
